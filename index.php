...
.hf-img{object-fit:contain;display:block;filter:drop-shadow(0 20px 40px rgba(0,0,0,.45))}
...
.hero-tag{display:inline-flex;align-items:center;gap:10px;background:var(--white);color:var(--esp);font-family:'Playfair Display',serif;font-style:italic;font-size:1rem;letter-spacing:.02em;padding:14px 32px;border-radius:6px;margin-top:32px;box-shadow:0 8px 30px rgba(0,0,0,.3);cursor:none;transition:transform .3s var(--ease),box-shadow .3s;opacity:0;animation:fadeUp .6s 1.05s forwards}
...
.ticker-item{display:flex;align-items:center;gap:20px;padding:0 22px;white-space:nowrap;color:var(--gold);font-size:.7rem;font-weight:500;letter-spacing:.14em;text-transform:uppercase;opacity:.92;transition:all .3s ease}
.ticker-dot{width:5px;height:5px;border-radius:50%;background:var(--gold);animation:pulse 2s infinite}
...
.c-card{display:flex;flex-direction:column;align-items:center;cursor:none;opacity:0;transform:translateY(32px)}
...
.c-img-wrap{width:clamp(160px,18vw,230px);height:clamp(160px,18vw,230px);position:relative;flex-shrink:0;border-radius:50%;overflow:hidden;transition:transform .45s var(--ease),box-shadow .45s var(--ease)}
.c-card:hover .c-img-wrap{transform:scale(1.08) translateY(-8px);box-shadow:0 24px 60px rgba(74,37,16,.28)}
.c-img-wrap img{width:100%;height:100%;object-fit:cover;border-radius:50%;transition:transform .6s var(--es)}
.c-card:hover .c-img-wrap img{transform:scale(1.08)}
.c-stok{position:absolute;top:10px;right:-10px;z-index:20;font-size:.58rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;padding:7px 13px;border-radius:999px;box-shadow:0 8px 20px rgba(0,0,0,.18)}
...
.c-add{margin-top:12px;display:inline-flex;align-items:center;gap:7px;padding:8px 20px;border-radius:var(--pill);background:var(--esp);color:var(--white);font-size:.75rem;font-weight:500;opacity:0;transform:translateY(6px);transition:opacity .3s,transform .3s,background .3s}
...
.favmenu-section{padding:96px 64px 100px;background:var(--white);position:relative;overflow:hidden}
.favmenu-section::before{content:'';position:absolute;width:600px;height:600px;border-radius:50%;top:-200px;right:-180px;background:radial-gradient(circle, rgba(245,206,204,.35) 0%, transparent 65%);pointer-events:none}
.favmenu-section::after{content:'';position:absolute;width:400px;height:400px;border-radius:50%;bottom:-150px;left:-120px;background:radial-gradient(circle, rgba(232,180,109,.12) 0%, transparent 65%);pointer-events:none}
.favmenu-head{text-align:center;margin-bottom:16px;position:relative;z-index:1}
.favmenu-head .stag{display:block;font-size:.68rem;font-weight:500;letter-spacing:.22em;text-transform:uppercase;color:var(--car);margin-bottom:10px}
.favmenu-head .sh{font-family:'Playfair Display',serif;font-size:clamp(36px,5vw,62px);font-weight:900;color:var(--esp);line-height:1.05}
.favmenu-head .sh em{font-style:italic;color:var(--brown)}
.favmenu-sub{text-align:center;font-size:.92rem;line-height:1.8;color:var(--muted);max-width:520px;margin:0 auto 68px;font-style:italic;position:relative;z-index:1}
.favmenu-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:16px 32px;position:relative;z-index:1}
.fav-card{display:flex;flex-direction:column;align-items:center;cursor:none;opacity:0;transform:translateY(28px)}
...
.fav-img-ring{width:clamp(180px,20vw,260px);height:clamp(180px,20vw,260px);border-radius:50%;background:var(--blush);display:flex;align-items:center;justify-content:center;overflow:hidden;position:relative;transition:transform .45s var(--ease),box-shadow .45s;box-shadow:0 8px 32px rgba(74,37,16,.12)}
.fav-card:hover .fav-img-ring{transform:translateY(-10px) scale(1.05);box-shadow:0 22px 52px rgba(74,37,16,.22)}
.fav-img-ring img{width:100%;height:100%;object-fit:cover;transition:transform .6s var(--es)}
.fav-card:hover .fav-img-ring img{transform:scale(1.1)}
.fav-rank{position:absolute;top:14px;left:14px;width:28px;height:28px;border-radius:50%;background:var(--esp);color:var(--gold);font-size:.65rem;font-weight:700;display:flex;align-items:center;justify-content:center}
.fav-name{font-family:'Playfair Display',serif;font-size:1.1rem;font-weight:700;color:var(--esp);text-align:center;margin-top:22px;line-height:1.2}
.fav-price{font-family:'Playfair Display',serif;font-size:1.05rem;font-style:italic;color:var(--car);margin-top:5px}
.fav-stars{display:flex;gap:2px;margin-top:6px}
.fav-stars span{color:var(--gold);font-size:.72rem}
.favmenu-divider{display:flex;align-items:center;gap:18px;max-width:320px;margin:60px auto 0;position:relative;z-index:1}
.favmenu-divider::before,.favmenu-divider::after{content:'';flex:1;height:1px;background:var(--cream2)}
.favmenu-divider-text{font-family:'Kaushan Script',cursive;font-size:1.1rem;color:var(--muted);white-space:nowrap}
.testi-section{background:var(--esp-2);overflow:hidden;position:relative}
.testi-slide{min-height:75vh;display:grid;grid-template-columns:360px 1fr 360px;align-items:center;padding:80px 0;position:relative}
.testi-slide:nth-child(even){background:rgba(0,0,0,.12)}
.testi-food{width:100%;height:100%;display:flex;align-items:flex-end;overflow:visible;position:relative}
.testi-food.left{justify-content:flex-end}
.testi-food.right{justify-content:flex-start}
.testi-food img{width:clamp(250px,28vw,380px);height:clamp(300px,36vw,460px);object-fit:contain;flex-shrink:0;filter:drop-shadow(0 24px 60px rgba(0,0,0,.45));transition:transform .6s var(--es),filter .6s var(--es)}
...
.testi-section-label{position:absolute;top:44px;left:64px;font-family:'Kaushan Script',cursive;font-size:1.45rem;color:rgba(255,252,247,.3)}
.testi-center{padding:0 40px;text-align:center;display:flex;flex-direction:column;align-items:center;justify-content:center;position:relative;z-index:1}
.testi-sub-label{font-family:'Kaushan Script',cursive;font-size:clamp(1.2rem,2.5vw,1.8rem);color:rgba(255,252,247,.4);margin-bottom:20px}
.testi-quote-text{font-family:'Playfair Display',serif;font-size:clamp(22px,3.5vw,46px);font-weight:700;line-height:1.2;color:var(--white);margin-bottom:28px}
...
.testi-bottom-label{text-align:center;padding:40px;font-family:'Kaushan Script',cursive;font-size:clamp(28px,5vw,56px);color:rgba(255,252,247,.12);border-top:1px solid rgba(255,252,247,.06)}
...
.testi-add-wrap{text-align:center;padding:50px 20px 70px}
.testi-add-btn{display:inline-flex;align-items:center;justify-content:center;gap:10px;padding:15px 34px;border-radius:999px;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.12);backdrop-filter:blur(10px);color:var(--white);font-size:.85rem;font-weight:500;letter-spacing:.08em;text-transform:uppercase;transition:all .35s var(--es);text-decoration:none}
.testi-add-btn:hover{background:var(--gold);color:var(--esp);transform:translateY(-4px);box-shadow:0 14px 30px rgba(232,180,109,.28)}
.review-success{width:fit-content;margin:30px auto 10px;padding:14px 26px;border-radius:999px;background:rgba(232,180,109,.12);border:1px solid rgba(232,180,109,.25);color:var(--gold);font-size:.9rem;font-weight:500;letter-spacing:.03em;backdrop-filter:blur(10px);animation:fadeNotif .5s ease}
@keyframes fadeNotif{from{opacity:0;transform:translateY(-10px)}to{opacity:1;transform:translateY(0)}}
@media(max-width:960px){
  .nav{padding:14px 22px}.nav.stuck{padding:12px 22px}.nav-links{display:none}
  .hero{padding:100px 22px 80px}.hf-wrap.ml,.hf-wrap.mr{display:none}
  .hf-wrap.tl .hf-img,.hf-wrap.tr .hf-img{width:120px;height:120px}
  .hf-wrap.bl .hf-img,.hf-wrap.br .hf-img{width:110px;height:110px}
  .menu-section{padding:70px 22px 80px}.circle-grid{grid-template-columns:repeat(2,1fr);gap:24px 16px}
  .favmenu-section{padding:70px 22px 80px}.favmenu-grid{grid-template-columns:repeat(2,1fr);gap:24px 16px}
  .testi-slide{grid-template-columns:1fr;padding:60px 22px}.testi-food{display:none}
  .testi-section-label{left:22px}.gallery-section{padding:70px 0}
  .gallery-head{padding:0 22px 32px}.gallery-track{padding:0 22px 10px}
  footer{padding:56px 22px 30px}.footer-grid{grid-template-columns:1fr 1fr;gap:32px}
...
<nav class="nav" id="mainNav">
  ...
  <div class="nav-r">
    <button class="nav-icon" title="Keranjang" onclick="alert('Silakan login atau buat akun terlebih dahulu!'); window.location.href='login.php'">
      <svg viewBox="0 0 24 24"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg>
      <span class="cart-badge" id="cartBadge">0</span>
    </button>
    <button class="nav-icon" title="Akun" onclick="alert('Anda belum login!'); window.location.href='login.php'">
...
<section class="hero" id="beranda">
  <div class="hf-wrap tl"><img class="hf-img" src="gambar/nastar.png" alt=""></div>
  <div class="hf-wrap tr"><img class="hf-img" src="gambar/kastengel.png" alt=""></div>
  <div class="hf-wrap bl"><img class="hf-img" src="gambar/strawberry_thumbprint.png" alt=""></div>
  <div class="hf-wrap br"><img class="hf-img" src="gambar/lidah_kucing.png" alt=""></div>
  <div class="hf-wrap ml"><img class="hf-img" src="gambar/kue_kacang.png" alt=""></div>
  <div class="hf-wrap mr"><img class="hf-img" src="gambar/sagu_keju.png" alt=""></div>
  ...
  <div class="hero-inner">
    ...
    <div class="hero-stats">
      <div class="stat">
        <span class="stat-n"><span class="ctr" data-to="<?= $dMenu['total_menu']; ?>">0</span><sup>+</sup></span>
      ...
      <div class="stat">
        <span class="stat-n"><span class="ctr" data-to="<?= $dPelanggan['total_pelanggan']; ?>">0</span><sup>+</sup></span>
...
<section class="menu-section" id="menu">
  ...
  <div class="circle-grid" id="circleGrid">
    ...
    while($data = mysqli_fetch_assoc($query)){
      $stok = $data['stok'];
      if($stok > 20){ $class_stok="stok-aman"; $text_stok="Stok $stok"; }
      elseif($stok > 0){ $class_stok="stok-tipis"; $text_stok="Stok Tipis"; }
      else{ $class_stok="stok-habis"; $text_stok="Stok Habis"; }
    ?>
    <div class="c-card">
      <div class="c-img-wrap">
        <img src="gambar/<?= $data['gambar']; ?>" alt="<?= $data['nama_produk']; ?>" loading="lazy">
        <span class="c-stok <?= $class_stok ?>"><?= $text_stok ?></span>
      </div>
      <span class="c-cat">
        <?php
        if($data['kategori_id']==1) echo "Kue Kering";
        elseif($data['kategori_id']==2) echo "Kue Basah";
        elseif($data['kategori_id']==3) echo "Cookies";
        elseif($data['kategori_id']==4) echo "Cake & Brownies";
        elseif($data['kategori_id']==5) echo "Snack Manis";
        else echo "Kategori Tidak Ada";
        ?>
      </span>
      <h3 class="c-name"><?= $data['nama_produk']; ?></h3>
      <span class="c-price">Rp <?= number_format($data['harga'],0,',','.'); ?></span>
      <button class="c-add" onclick="alert('Silakan login atau buat akun terlebih dahulu!'); window.location.href='login.php'">
  ...
  <div class="see-all-wrap reveal">
    <a href="login.php" class="see-all" onclick="alert('Silakan login atau buat akun terlebih dahulu!')">
      Pesan Sekarang
      <svg viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6"/></svg>
    </a>
  </div>
...
<section class="favmenu-section">
  ...
  <p class="favmenu-sub reveal">Produk terlaris pilihan pelanggan dengan rating terbaik dari toko kami ✨</p>
  <div class="favmenu-grid" id="favGrid">
    <?php
    $fav = mysqli_query($koneksi, "
      SELECT produk.produk_id, produk.nama_produk, produk.harga, produk.gambar,
        COALESCE(SUM(order_items.qty),0) as total_terjual,
        COALESCE(AVG(review.rating),0) as rata_rating
      FROM produk
      LEFT JOIN order_items ON produk.produk_id = order_items.produk_id
      LEFT JOIN review ON produk.produk_id = review.produk_id AND review.status='tampil'
    ...
    <div class="fav-card">
      <div class="fav-img-ring">
        ...
        <img src="gambar/<?= $f['gambar']; ?>" alt="<?= $f['nama_produk']; ?>" loading="lazy">
      </div>
      <h3 class="fav-name"><?= $f['nama_produk']; ?></h3>
      <span class="fav-price">Rp <?= number_format($f['harga'],0,',','.'); ?></span>
      <div class="fav-stars">
        <?php
        $r = round($f['rata_rating']);
        for($i=1;$i<=5;$i++) echo $i<=$r ? "<span>★</span>" : "<span>☆</span>";
        ?>
      </div>
    </div>
    <?php } ?>
  ...
  <div class="favmenu-divider reveal">
    <span class="favmenu-divider-text">Favorit pelanggan kami ✨</span>
  </div>
</section>
...
<section class="testi-section" id="tentang">
  ...
  <div class="review-success">✨ Review berhasil ditambahkan, terima kasih!</div>
  <?php } ?>
  ...
  $qReview = mysqli_query($koneksi, "
    SELECT review.*, produk.gambar, produk.gambar_testi
    FROM review
  ...
  <div class="testi-slide">
    ...
    <div class="testi-center">
      ...
      <p class="testi-quote-text">"<?= $r['review']; ?>"</p>
      <div class="testi-stars">
        <?php for($i=1;$i<=5;$i++) echo $i<=$r['rating'] ? "<span>★</span>" : "<span>☆</span>"; ?>
      </div>
      <span class="testi-name"><?= $r['nama_pelanggan']; ?></span>
    </div>
  ...
  <div class="testi-add-wrap">
    <a href="login.php" class="testi-add-btn" onclick="alert('Silakan login atau buat akun terlebih dahulu!')">
