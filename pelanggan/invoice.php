...
// Cek apakah metode Cash
$isCash = strtolower($order['metode_pembayaran']) === 'cash';
?>
...
<title>Struk <?= htmlspecialchars($order['invoice']) ?></title>
<style>
@media print { header, footer { display:none !important; } }
* { margin:0; padding:0; box-sizing:border-box; }
body { background:#f5f5f5; font-family:'Courier New', monospace; padding:25px; }
.struk { width:340px; margin:auto; background:#fff; padding:20px; border-radius:6px; box-shadow:0 4px 18px rgba(0,0,0,.08); position:relative; }
.struk::before, .struk::after { content:""; position:absolute; left:0; width:100%; height:12px; background:radial-gradient(circle, transparent 7px, #fff 8px); background-size:20px 20px; }
.struk::before { top:-10px; }
.struk::after { bottom:-10px; transform:rotate(180deg); }
.header { text-align:center; border-bottom:1px dashed #999; padding-bottom:14px; margin-bottom:14px; }
.logo { font-size:22px; font-weight:bold; }
.tagline { font-size:12px; color:#777; margin-top:4px; }
.alamat { font-size:12px; margin-top:6px; color:#777; }
.info { font-size:12px; margin-bottom:15px; }
.info-row { display:flex; justify-content:space-between; margin-bottom:5px; }
.status { font-weight:bold; color:<?= $statusInfo['color'] ?>; }
.items { border-top:1px dashed #999; border-bottom:1px dashed #999; padding:12px 0; }
.item { margin-bottom:14px; }
.item:last-child { margin-bottom:0; }
.item-name { font-size:13px; font-weight:bold; }
.item-detail { display:flex; justify-content:space-between; font-size:12px; margin-top:4px; }
.total-box { margin-top:14px; }
.total-row { display:flex; justify-content:space-between; font-size:13px; margin-bottom:7px; }
.grand-total { border-top:1px dashed #999; padding-top:10px; margin-top:8px; font-size:15px; font-weight:bold; }
.payment { border-top:1px dashed #999; margin-top:15px; padding-top:12px; }
.payment h4 { font-size:13px; margin-bottom:10px; }
...
.footer { text-align:center; margin-top:18px; border-top:1px dashed #999; padding-top:14px; }
.footer p { font-size:12px; margin-bottom:4px; }
.footer small { color:#777; font-size:11px; }
.action { text-align:center; margin-top:25px; }
.btn { display:inline-block; padding:10px 18px; border:none; border-radius:8px; cursor:pointer; text-decoration:none; font-size:13px; margin:5px; }
.btn-back { background:#eaeaea; color:#333; }
.btn-print { background:#2d1b10; color:#fff; }
@media print {
    header, footer, .navbar, .sidebar, .topbar { display:none !important; }
    body { background:#fff; padding:0; margin:0; }
    .action { display:none; }
    .struk { width:100%; max-width:340px; margin:auto; box-shadow:none; border-radius:0; border:1.5px dashed #999 !important; padding:15px; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
}
</style>
</head>
<body>
...
>>>>>>> ce5bdb00b2118cf11ff4795877541f55c33b7be4
    ...
    <div class="items">
        ...
        <div class="item">
            ...
            <div class="item-detail">
                <span><?= $d['qty'] ?> x <?= rupiah($d['harga']) ?></span>
    ...
    <!-- Bagian pembayaran hanya tampil kalau Cash -->
    <?php if($isCash && $pembayaran): ?>
    <div class="payment">
        <h4>Pembayaran</h4>
...
<div class="action">
    <a href="akun.php" class="btn btn-back">← Kembali</a>
